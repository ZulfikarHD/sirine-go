# 🏗️ Architecture

Dokumentasi untuk memahami struktur, design decisions, dan tech stack aplikasi.

---

## 📚 Files di Folder Ini

### **[overview.md](./overview.md)** 🏛️
Penjelasan lengkap semua package dan kenapa dibutuhkan.
- Backend packages (Go/Gin/GORM)
- Frontend packages (Vue/Vite/Tailwind)
- Service Pattern architecture
- PWA & Offline strategy

**Cocok untuk:** Memahami "why" di balik tech stack choices.

---

### **[folder-structure.md](./folder-structure.md)** 📁
Struktur folder dan file organization detail.
- Backend folder structure
- Frontend folder structure
- Data flow (request/response)
- File naming conventions

**Cocok untuk:** Navigasi codebase dengan mudah.

---

### **[project-summary.md](./project-summary.md)** 📊
Complete overview of the entire project.
- Features list
- Tech stack summary
- Performance metrics
- Design decisions
- Completion status

**Cocok untuk:** Big picture understanding dan onboarding.

---

## 🎯 Recommended Flow

```
1. project-summary.md (big picture)
      ↓
2. overview.md (understand tech)
      ↓
3. folder-structure.md (know where things are)
      ↓
4. ✅ Ready to customize!
```

---

## 🔗 Related Documentation

**After understanding architecture:**
- Build features: [../development/customization-guide.md](../03-development/customization-guide.md)
- API reference: [../development/api-documentation.md](../03-development/api-documentation.md)

**Questions about architecture?**
- Check: [../troubleshooting/faq.md](../09-troubleshooting/faq.md)

---

## 📂 Folder Navigation

← Back to [Documentation Root](../README.md)  
← Previous: [Getting Started](../01-getting-started/README.md)  
→ Next: [Development Documentation](../03-development/README.md)

---

**Last Updated:** 27 Desember 2025
