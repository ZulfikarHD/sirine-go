  ...
  <AppLayout>
    <div class="max-w-2xl mx-auto">
      <Motion
        ...
        class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 space-y-6"
      >
        <!-- Header -->
        ...
        >
          ...
          <div>
            <p class="font-semibold text-green-900">Password Berhasil Diubah!</p>
            <p class="text-sm text-green-700 mt-1">
              Anda akan diarahkan ke halaman login untuk login kembali dengan password baru...
            </p>
          </div>
        </Motion>
        ...
        <form v-if="!isSuccess" @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div class="flex gap-3 pt-4">
            <button
              type="button"
              class="flex-1 px-6 py-3 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold focus:outline-none focus:ring-4 focus:ring-gray-100 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.97]"
              @click="router.back()"
              :disabled="isLoading"
            >
              Batal
...
/**
 * ChangePassword page untuk user mengubah password sendiri
 * dengan validasi current password dan password strength indicator
 */
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Auto logout handled by composable
  } catch (err) {
